  ...
  return (
    <div style={{
      ...
      <h2 style={{ fontSize: '32px', fontWeight: 800, marginBottom: '16px', color: '#fff', letterSpacing: '-0.02em', lineHeight: 1.3 }}>
        {configs.title || 'Quà Xinh'}
      </h2>
      <p style={{ fontSize: '16px', lineHeight: 1.7, opacity: 0.9, maxWidth: '750px', margin: '0 auto 28px', color: '#fceaf0' }}>
        {configs.description || ''}
      </p>
    ...
    <div style={{ padding: '32px 0', marginBottom: '32px' }}>
      {configs.title && (
        <div style={{ textAlign: 'center', marginBottom: '36px' }}>
          {configs.tag && (
            <span style={{ fontSize: '12px', fontWeight: 800, color: '#b85c78', textTransform: 'uppercase', letterSpacing: '0.08em', display: 'block', marginBottom: '8px' }}>
              {configs.tag}
            </span>
          )}
          <h3 style={{ fontSize: '26px', fontWeight: 800, color: '#4a2a38' }}>{configs.title}</h3>
          {configs.description && (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '24px' }}>
        {items.map((item, idx) => (
          <div key={idx} style={{ background: '#ffffff', border: '1px solid #e5e7eb', borderRadius: '16px', padding: '24px', boxShadow: '0 4px 16px rgba(0,0,0,0.02)', display: 'flex', gap: '16px', alignItems: 'flex-start' }}>
            ...
            <div>
              <h4 style={{ fontSize: '16px', fontWeight: 700, color: '#4a2a38', margin: '0 0 8px 0' }}>{item.title}</h4>
              ...
              {listPosts.map((post, i) => (
                <div key={i} className="lc-blog-row">
                  ...
                  <div className="lc-blog-row-info">
                    ...
                    <span>{post.author_name || 'Quà Xinh'} · {new Date(post.created_at).toLocaleDateString('vi-VN')}</span>
                  </div>
                </div>
              ))}
            </div>
