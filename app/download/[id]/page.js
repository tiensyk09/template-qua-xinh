...
export async function generateMetadata({ params }) {
  ...
  if (!decoded) return { title: 'Asset Not Found | Quà Xinh' };
  ...
  if (!file || file.is_public === 0) {
    return { title: 'Asset Restricted | Quà Xinh' };
  }

  return {
    title: `Download: ${file.name} | Quà Xinh`,
    description: file.description || `Download asset ${file.name} securely from Quà Xinh.`
  };
}
