...
export async function generateMetadata({ searchParams }) {
  ...
  let title = 'Tất cả sản phẩm | Quà Xinh';
  if (categorySlug) {
    const cats = await query('SELECT name FROM shop_categories WHERE slug = ?', [categorySlug]);
    if (cats.length > 0) {
      title = `${cats[0].name} | Quà Xinh`;
    }
  } else if (searchQ) {
    title = `Tìm kiếm: "${searchQ}" | Quà Xinh`;
  }
  
  return {
    title,
    description: 'Mua phụ kiện thời trang, móc khóa, kẹp tóc nơ, vòng tay charm, đèn ngủ, sổ tay và quà tặng xinh xắn chính hãng tại Quà Xinh.'
  };
}
